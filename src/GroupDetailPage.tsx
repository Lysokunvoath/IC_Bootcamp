...
export default function GroupDetailPage({ groups, setGroups, meetups, selectedGroupId, userId, setCurrentPage, navigateToAddActivity }) {
    ...
    const handleTogglePrivate = () => {
        ...
        setMessage(`Group "${group.name}" is now ${group.isPublic ? 'private' : 'public'}.`);
        setTimeout(() => setMessage(''), 3000);
    };

    const upcomingMeetups = meetups.filter(m => m.groupId === selectedGroupId && new Date(m.date) > new Date());
        ...
        <div className="min-h-screen bg-gray-100 p-8 text-gray-800 font-sans">
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center space-x-4 text-gray-600">
                    <button onClick={handleAddToFavorites} className="flex items-center space-x-2 text-[#34699A] hover:text-[#113F67] transition-colors">
                         <span>Add to favourite</span>
                         <Star size={20} className="stroke-current" />
                    </button>
                    <button onClick={handleCopy} className="p-2 bg-white rounded-xl shadow-md hover:bg-gray-100 transition-colors">
                ...
                <div className="md:col-span-2">
                    <div className="bg-[#34699A] text-white p-6 rounded-xl shadow-lg">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            {isMember && (
                                 <button onClick={handleAddActivityClick} className="bg-[#113F67] text-white px-4 py-2 rounded-xl shadow-lg hover:bg-green-500 transition-colors">
                                    Add
                                </button>
                            )}
                        </div>
                        ...
                        {upcomingMeetups.length > 0 ? (
                            <div className="space-y-4">
                                {upcomingMeetups.map(meetup => (
                                    <div key={meetup.id} className="grid grid-cols-3 text-md text-gray-200">
                    ...
                    <div className="p-6">
                        ...
                        <div className="space-y-2 mb-6">
                            {members.map((memberId, index) => (
                                <p key={index} className="text-lg font-medium">{memberId === userId ? 'You' : memberId}</p>
                            ))}
                        </div>
                        {isOwner && (
                            <>
                                <button onClick={() => setAddMemberModalOpen(true)} className="bg-[#34699A] text-white px-6 py-3 rounded-xl shadow-lg hover:bg-[#113F67] transition-colors">
                                    Add member
                                </button>
