...
export default function JoinGroupPage({
  ...
  const publicGroups = groups.filter((group: any) => group.isPublic);

  const filteredGroups = publicGroups.filter((group: any) =>
        ...
        <div className="space-y-4">
          {filteredGroups.length > 0 ? (
            filteredGroups.map((group: any) => (
              <div
                key={group.id}
                className="bg-[#34699A] text-white p-4 rounded-xl shadow-md flex justify-between items-center"
              >
